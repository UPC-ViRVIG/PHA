...
class MARL4ExperienceBuffer(ExperienceBuffer):
    ...
    def _init_from_env_info(self, env_info):
        ...
        if self.is_discrete or self.is_multi_discrete:
            assert self.is_discrete, "discrete not supported yet!"
            assert self.is_multi_discrete, "multi_discrete not supported yet!"
        if self.use_action_masks:
            assert self.use_action_masks, "action masks not supported yet!"
